...
const viewType = 'biztalkBamViewer.editor';
...
function activate(context) {
  const provider = vscode.window.registerCustomEditorProvider(viewType, {
    resolveCustomTextEditor(document, panel) {
      ...
      const send = () => panel.webview.postMessage({ type: 'openBam', fileName: path.basename(document.uri.fsPath || document.uri.path), source: document.getText() });
  ...
  const command = vscode.commands.registerCommand('biztalkBamViewer.open', async uri => {
    const target = uri || (await vscode.window.showOpenDialog({ canSelectMany: false, filters: { 'BizTalk BAM definitions': ['xml', 'bam'] } }))?.[0];
